...
print(word.remove(at: word.index(before: word.endIndex)))
//: __Problem 2__
//:
...
func combineLastCharacters(strArr: [String]) -> String {
  var retval = ""
  for var str in strArr {
    retval.append(str.remove(at: str.index(before: str.endIndex)))
  }
  return retval
}

print(combineLastCharacters(strArr: nonsenseArray))
//: __Problem 3__
//:
...
func cleanUp(_ dirtyWord: [String]) -> [String] {
...
class MovieArchive {
  ...
  func filterByDirector(_ searchDirector: String, movies: Dictionary<String, String>) -> [String] {
    var foundMovies = [String]()
    for (movie, director) in movies {
      if director == searchDirector {
